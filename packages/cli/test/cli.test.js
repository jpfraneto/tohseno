    ...
    artifacts: [
      {
        ...
        url: "https://github.com/jpfraneto/tohseno/releases/download/v1.2.0/tohseno-release-aarch64-apple-darwin.tar.gz",
        byte_size: 123,
        sha256: "ab".repeat(32),
      ...
      },
      {
        ...
        url: "https://github.com/jpfraneto/tohseno/releases/download/v1.2.0/tohseno-release-x86_64-apple-darwin.tar.gz",
        byte_size: 456,
        sha256: "cd".repeat(32),
        signing: { kind: "release-package", team_id: null, designated_requirement: null },
      },
...
test("manifest selects the exact architecture and enforces minimum CLI", () => {
  assert.equal(validateManifest(manifest(), "1.2.0", "arm64").artifact.target, "aarch64-apple-darwin");
  const newer = manifest();
  newer.minimum_npm_cli_version = "1.2.1";
  assert.throws(() => validateManifest(newer, "1.2.0", "arm64"), /too old/);
  assert.throws(() => nodeArchitecture("mips"), /Apple silicon and Intel/);
});

test("npm 1.2.0 refuses a stale native release manifest", () => {
  const stale = manifest();
  stale.native_release_version = "1.0.0";
  assert.throws(
    () => validateManifest(stale, "1.2.0", "arm64"),
    /requires native TOHSENO 1\.2\.0/,
  );
});
...
test("manifest rejects duplicate architectures, sizes, digests, and extra fields", () => {
  ...
  assert.throws(() => validateManifest(duplicate, "1.2.0", "arm64"), /duplicate/);
  const size = manifest();
  size.artifacts[0].byte_size = 0;
  assert.throws(() => validateManifest(size, "1.2.0", "arm64"), /byte size/);
  const digest = manifest();
  digest.artifacts[0].sha256 = "AB".repeat(32);
  assert.throws(() => validateManifest(digest, "1.2.0", "arm64"), /SHA-256/);
  const extra = manifest();
  extra.token = "secret";
  assert.throws(() => validateManifest(extra, "1.2.0", "arm64"), /unexpected/);
});
