---
title: Intelligence and managed compute
...
Managed inference is optional. The server provides priced estimates and caps. Use requires explicit consent, an installation-signed managed claim, sufficient append-only micro-USD balance, and a hard reservation before the constrained completion proxy can call the allowlisted provider/model route.
...
Configuration code existing in the repository does not make managed compute live. The route remains unavailable unless the exact provider key, model allowlist, server durability, estimates/caps, reservation behavior, webhook verification, backup/restore, and scenario matrix are operationally proven.

When managed compute is unavailable, the product may explain how to configure a local/BYO agent. It must not silently switch routes, spend balance without consent, or reinterpret a failed reservation as permission.

See [current status](/guide/reference/current-status/) for today's deployment evidence.
