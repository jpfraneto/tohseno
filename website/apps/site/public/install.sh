...
if [ "$macos_major" -lt 13 ]; then
  fail "TOHSENO 0.7.0 requires macOS 13 or later."
fi
...
verify_artifact "$materials_name"
  ...
  if [ "$installed_version" != "tohseno 0.7.0" ]; then
    fail "The downloaded executable is not the pinned 0.7.0 release."
  fi
  ...
  if [ "$helper_version" != "tohseno-apple-identity 0.7.0" ]; then
    fail "The downloaded Apple identity helper is not the pinned 0.7.0 release."
  fi
