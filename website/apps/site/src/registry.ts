...
export async function createRegistryRouter(
  ...
  return {
    ...
    renderRegistry: async (rawQuery) => {
      ...
      if (query && (query.length > 100 || /[\u0000-\u001f\u007f]/.test(query))) return registryHTML([], "");
      const records = (await discoverable(await allRecords(directories.releases)))
        .filter((record) => !query || searchableRelease(releaseOf(record)).includes(query))
        .sort(newestFirst).slice(0, 100).map(publicRecord);
      return registryHTML(records, rawQuery?.trim() ?? "");
    },
...
function registryHTML(records: JsonObject[], query = ""): string { return page("The Registry", `<header><p class="eyebrow">PERSON-TO-PERSON NATIVE SOFTWARE</p><h1>Apps people made<br>for other people.</h1><p class="lead">Every app here is source-published, signed by its builder, witnessed on Robinhood Chain, and rebuilt on the recipient’s Mac.</p><form class="search" action="/registry" method="get"><label for="registry-search">Search apps, builders, or ShotID</label><div><input id="registry-search" name="q" maxlength="100" value="${escapeHTML(query)}"><button type="submit">Search</button></div></form></header>${cards(records)}`); }
...
function cards(records: JsonObject[]): string { if (!records.length) return `<section class="empty"><h2>The network is ready.</h2><p>The first independently verified release will appear here.</p></section>`; return `<main class="grid">${records.map((record) => { const release = object(record.release, "release"); const display = object(release.display, "display"); return `<a class="card" href="${escapeHTML(String(record.route))}"><p class="eyebrow">SHOT ${escapeHTML(String(release.checkpoint_sequence))}</p><h2>${escapeHTML(String(display.name))}</h2><p>${escapeHTML(String(display.description))}</p><span>Open app →</span></a>`; }).join("")}</main>`; }
...
function unavailableRouter(): RegistryRouter { return { handles: (path) => path.startsWith("/api/registry/v1/"), fetch: async () => json({ error: "The public Registry is not enabled." }, 503), renderRegistry: async () => registryHTML([]), renderShot: async () => undefined, renderBuilder: async () => undefined, renderHumanRoute: async () => undefined }; }
