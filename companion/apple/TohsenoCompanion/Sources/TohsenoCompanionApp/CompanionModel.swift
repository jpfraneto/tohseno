...
public final class CompanionModel {
    ...
    public private(set) var screen: Screen = .firstRun
    ...
    private func load() async {
        do {
            ...
            if case .firstRun = screen { screen = .apps }
            try await loadIcons()
        } catch TohsenoCompanionError.notPaired {
            screen = .firstRun
        } catch {
    ...
    private func loadIcons() async throws {
        for shot in apps {
            guard let descriptor = shot.icon, icons[shot.shotID] == nil else { continue }
            if let bytes = try await backend.iconBytes(for: descriptor) {
                icons[shot.shotID] = bytes
            }
        }
    }
    ...
    public func pair(scanned invitation: String) async {
        ...
        do {
            ...
            notice = "That code didn’t work. Show a new one on your Mac and scan again."
        }
    }
}
