...
The loop is deliberately small: create shot 1, use the app, then run `tohseno evolve <app-name>` for shot 2. `tohseno refresh` makes free Apple ID expiry routine, `tohseno list` shows the local ledger, and `tohseno studio` opens a simulator-first app library and the same press on localhost.

Every shot is a complete, append-only Xcode project. Its integer shot number is its `CFBundleVersion`; the filesystem under `~/.tohseno/` is the database; generated apps are iOS 17+ SwiftUI with no third-party dependencies; and installation is USB-only through Apple’s tools. Studio and the CLI detect local Hermes, Codex, Claude Code, Grok Build, and OpenCode installations, and remember the chosen harness in `~/.tohseno/config.toml`.
